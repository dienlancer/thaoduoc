...
class ModuleArticle extends WP_Widget {
	...
	public function widget( $args, $instance ) {			
		...
		if($status_wrapper=="active"){
			...
			require plugin_dir_path(__FILE__) . '/html/module-article.php';		
			echo $after_widget;	
		}
		else
			require plugin_dir_path(__FILE__) . '/html/module-article.php';				
	...
	public function update( $new_instance, $old_instance ) {			 
		...
		$instance['article_id'] 	= $new_instance['article_id'];					
	...
	public function form( $instance ) {	
		...
		$inputID 	= $this->get_field_id('article_id');
		$inputName 	= $this->get_field_name('article_id');
		$inputValue = @$instance['article_id'];
		$class = array("widefat");
		$html		= $vHtml->label('ArticleID',array('for'=>$inputID))
					. $vHtml->cmsTextbox($inputID,$inputName,"widefat",$inputValue);
		echo $vHtml->pTag($html);	
